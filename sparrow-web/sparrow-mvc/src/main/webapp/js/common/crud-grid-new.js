var _ImgEl, _ImgDlg;
var _ImgShow = function (v, n) {
    if (n == null) {
        n = 0;
    }
    if (_ImgDlg == null) {
        _ImgEl = $('<img src="/app/img/defaultx.png" />');
        _ImgDlg = PopDialog.pop({
            title: '图片显示',
            width: 650,
            height: 330,
            hideBtn: true,
    ...
    _ImgDlg.show(150, 460);
    _ImgEl.attr('ori', v);
    Common.lazyInit(_ImgEl);
};
var _Render = {
    _source: function (s) {
...
var _Editor = {
    ...
    'img': function (name, cfg) {
        var img = $('<img src="/app/img/defaultx.png" style="width:180px;height:260px;cursor:pointer;" />');
        if (cfg.imgEl)
    ...
    'picBrowse': function (name, cfg) {
        var div = cfg._cl;
        var img = $('<img src="/app/img/defaultx.png" style="width:270px;height:120px;"/>');
        if (cfg.imgEl)
            img.attr('id', cfg.imgEl);
        var el = $('<select name="' + name + '" ></select>');
        var v = el[0];
    ...
    'picShow': function (name, cfg) {
        var div =$('<div></div>');
        //var el = $('<input type="text" name="' + name + '" >');
        var el = $('<textarea name="' + name + '"/>');
        var btn = this.__createBtn('打开',function(evt,x){
            var v=el.val();
            if(v) {
...
_Form.getParas = function (el) {
    var fields = _Form.getFormFields(el);
    if (fields) {
        var paras = {};
