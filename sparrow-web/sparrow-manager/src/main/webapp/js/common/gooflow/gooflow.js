...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		var tp=_this.$nowType.split(" ")[0];
		var nm=_this.$nodeRemark[tp];
		_this.addNode(_this.$id+"_node_"+_this.$max,{name:nm,left:X,top:Y,type:_this.$nowType});		
...
GooFlow.prototype={
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			else if(json.type.indexOf(" err")>-1)  this.$nodeDom[id].addClass("item_err");
		}else{
			json.width=24;
			json.height=24;
	...
	initWorkForNode:function(){
		...
		//if(this.isBaseNode(this.$nowType) )	return;
		//绑定用鼠标移动事件
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			//	This.$textArea.val("").removeData("id").hide();
			//});
		  if(This.onTaskDblClick)
			  This.onTaskDblClick(This.$focus,This.$nodeData[id]);
		});
		//绑定结点的删除功能
