var _Render = {
    ...
    divApplyState: function (v) {

        var s = Common.getDit('divApplyState');
        if (typeof (v) == 'undefined' || v == null)
            return '';
        v = String(v);
        if (v && s) {
            if (v == 2)
                v = '<font color="#7fffd4">' + s[v] + '</font>';
            else if (v == 4)
                v = '<font color="#8a2be2">' + s[v] + '</font>';
            else if (v == 6)
                v = '<font color="#bdb76b""">' + s[v] + '</font>';
            else if (v == 7)
                v = '<font color="#4b0082">' + s[v] + '</font>';
            else if (v == 9)
    ...
    loginStatus: function (v) {
        if (v == 1) {
            v = '<font color="red">从未登录</font>';
        } else if (v == 0) {
            v = '<font color="blue">登录过</font>';
        }
        return v;
    },

    allowStatus: function (v) {
        if (v == 1) {
            v = '<font color="red">启用</font>';
        } else if (v == 0) {
            v = '<font color="blue">禁用</font>';
        }
        return v;
    },

    customStatus: function (v) {
        if (v == 0) {
            v = '<font color="red">新建</font>';
        } else if (v == 1) {
            v = '<font color="blue">运行中</font>';
        }
        return v;
    },

    customLogStatus: function (v) {
        if (v == 0) {
            v = '<font color="red">失败</font>';
        } else if (v == 1) {
            v = '<font color="blue">成功</font>';
        }
        return v;
    },
...
var _Editor = {
    ...
    'radiogroup': function (name, cfg) {

        var el = $('<div></div>');
        var values = cfg.values;
        var disable = cfg.disabled;
        var checkedValue = cfg.chkval;
        var str = values.split(",");
        for (i = 0; i < str.length; i++) {
            if (str[i].split("_")[0] == checkedValue) {
                var tmpVar = $('<input type="radio" name="' + name
                    + '"  value="' + str[i].split("_")[0]
                    + '"style="margin-top:0px"/>');
                tmpVar.prop("checked", true);
                if (disable) {
                    tmpVar.prop("disabled", true);
                }

                el.append(tmpVar);

                el.append($('<span style="margin-right:20px"> '
                    + str[i].split("_")[1] + '</span>'));
            } else {
                var tmpVar = $('<input type="radio" name="' + name + '"  value="'
                    + str[i].split("_")[0] + '" style="margin-top:0px"/>');
                if (disable) {
                    tmpVar.prop("disabled", true);
                }
                el.append(tmpVar);
    ...
    'desktopArea': function (name, cfg) {
        ...
        if (cfg.height) {
            var n = Common.getPxNum(cfg.height);
            /* n = n - 80; */
            height = n + 'px';
        }

        var el = $('<div></div>');
        var tel = $('<textarea id="' + name + '_lb" name="' + name
            + '_label" readonly="readonly">');
        tel.css({
            width: width,
            height: height
    ...
    'btnTxtEx': function (name, cfg) {
        var div = cfg._cl;
        var el = $('<input type="hidden" name="' + name + '" >');
        /*
         * if (cfg.ref) el.attr('ref', cfg.ref);
         */
        div.append(el);
        div.append('&nbsp;&nbsp;');
...
_Form.prototype = {
    ...
    painSearchRow: function (table, cols, form) {
        ...
        for (var i = 0; i < cols.length; i++) {
            ...
            if (this.skipCol(col))
                continue;
            var ignore = (typeof (col.ignoreHide) == 'boolean')
                && col.ignoreHide;
...
$.extend(_Panel.prototype, _Dialog.prototype, {
    ...
    drawFooter: function (gel) {
        ...
        if (btns.doSub) {
            var sbBtn = $('<button class="btn btn-primary">提交报表</button>');
        ...
        if (btns.doAuth) {
            var sbBtn = $('<button class="btn btn-primary">审核不通过</button>');
            cancelBtn.remove();
            el.append("&nbsp;&nbsp;").append(sbBtn);
            sbBtn.click(function () {
                btns.doAuth(_this);
        ...
        if (btns.doSyn) {
            var sbBtn = $('<button class="btn">同步实例</button>');
            cancelBtn.remove();
            el.append("&nbsp;&nbsp;").append(sbBtn);
            sbBtn.click(function () {
                btns.doSyn(_this);
...
var _PanelUser = _Create(_Constructor);
$.extend(_PanelUser.prototype, _Panel.prototype, {
    drawFooter: function (gel) {
        var btns = this.getButtons();
        btns = btns || {};
        if (btns.ok == 'x')
...
var _DPrototype = {
    ...
    createTable: function (cols, data, render) {
        ...
        for (var i = 0; i < cols.length; i++) {
            ...
            if (col.noe)
                continue;
            var ignore = (typeof (col.ignoreHide) == 'boolean') && col.ignoreHide;
            var needHide = col.hidden || col.noe;
            if (ignore && needHide) {
                continue;
            }
    ...
    createTableRv: function (cols, data, render) {
        var table = $('<table class="table table-striped table-bordered table-condensed table-hover"></table>');
        var mx = 3;
        var n = 0;
        var row;
...
var _CRUD_ = {
    ...
    createCboxCol: function (grid, ui, disabled) {
        return {
            _create: function () {
            ...
            if (typeof (col) == 'string' && _ui) {
                switch (col) {
                    ...
                    case 'x-':
                        ...
                        break;
                    case 'o':
                        col = _CRUD_.createRadioCol(_ui.grid, _ui, col);
                        break;
                }
                cols[i] = col;
