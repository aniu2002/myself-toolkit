...
LPage.mask = function (el) {
    ...
    var _html = "<div id='loadingMask' style='position:absolute;cursor:wait;top:"
        + offset.top + "px;left:" + offset.left + "px;width:" + width + "px;height:" + height
        + "px;background:#E0ECFF;opacity:0.6;filter:alpha(opacity=60);'>"
        + "<span style='position: relative; left:" + c_left + "px;top:" + c_top
    ...
    drawGridBar: function (total, page) {
        ...
        if (this.topPageBar == null) {
            ...
            if (this.opBar) {
                ...
                var obarEl = $('<div></div>');
                if (bel)
                    el.append(this.createTableRow(bel, obarEl));
                else {
                    this.topPageBar = true;
                    el.append(this.createTableRow(obarEl, null));
                }
                obarEl.append('&nbsp;');
                if (this.opBar.length) {
                    for (var i = 0; i < this.opBar.length; i++) {
        ...
        if (this.config.cols) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                if (col.render) {
                    ...
                    if (col._st) {
                        vl = data[this.idFeild];
                        if (this.valIn(vl, this._selVal))
                            v.attr('checked', true);
                        if (col.disabled) {
                            v.attr('checked', true);
