...
var Utils = {
    ...
    setting: function (el, url) {
        if (typeof (el) == 'string')
            el = $(el);
        Common.get(url, null, function (d) {
...
var Common = {
    loadHtml: '<div><img src="/app/img/ajax-loader.gif" />&nbsp;加载中...</div>',
    loadImg: '<img src="/app/img/ajax-loader.gif" />',
    _locks: {},
    _cb: {},
    _dic: {},
    store: {
        sqlType: [ 'mysql', 'oracle' ],
        webType: [ 'command', 'rest' ],
        provider: ['db', 'ws', 'http'],
        providerTypes: ['option', 'grid', 'tree'],
        bra: [ 'A', 'B', 'C', 'D', 'E', 'F' ],
        leval: [
            {code: '0', name: '不推荐'} ,
            {code: '1', name: '一般'},
    ...
    getUrlParas: function () {
        ...
        if (url.indexOf('?') != -1) {
            ...
            for (var i = 0; i < arr.length; i++) {
                var vp = arr[i].split("=");
                if (vp.length > 1)
                    theRequest[vp[0]] = decodeURIComponent(vp[1]);
                else
                    theRequest[vp[0]] = '';
            }
        }
    ...
    batchLoad: function (s, fn) {
        ...
        Common.get('/cmd/source?_s=' + s, null, handleResp, true);
    },
    loadSource: function (s, fn) {
        ...
        if (g == 'u')
            Common.get(s, null, handleResp, true);
        else if (g == 'f')
            Common.get('/app/json/' + s + '.json', null, handleResp, true);
        else
            Common.get('/cmd/source?_s=' + s + '&_e=' + g, null, handleResp, true);
    },
    ajaxLoad: function (s, fn) {
        var data = Common.store[s];
        if (data) {
            if (fn)
                fn(data);
            return;
    ...
    bindCBox: function (el, cfg) {
        var tf = typeof (cfg);
        if (tf == 'undefined')
            return;
        var nCfg = {};
        if (tf == 'string') {
            var da = Common.store[cfg];
            if (da)
                nCfg.data = da;
            else
                nCfg.url = cfg;
        } else if (tf == 'array' || cfg.length) {
            nCfg.data = cfg;
        } else
            nCfg = cfg;
        if (typeof (nCfg.url) == 'undefined' && typeof (cfg.data) == 'undefined')
            return;
        var xcb = function (d) {
            var sel = Common.renderOptions(el, d, nCfg.defVal, nCfg.onchange, nCfg);
            if (nCfg && nCfg.onSet) {
                var form = cfg.__fm;
                var opt = sel.options[sel.selectedIndex];
                cfg.onSet(opt.value, opt.val, sel, opt);
            }
        };
        if (nCfg.url)
            Common.ajaxLoad(nCfg.url, xcb);
        else if (nCfg.data)
            xcb(nCfg.data);
    },
    renderSelect: function (s, el, v, fn, cfg) {
        var xcb = function (d) {
            ...
            if (cfg && cfg.onSet) {
                ...
                cfg.onSet(opt.value, iv, sel, opt);
            }
        };
        if (typeof (s) == 'string')
    ...
    mask: function (el, msg) {
        ...
        var _inner = $('<div style="position: relative;width:180px;height:16px;padding:12px 5px 10px 30px;background:#fff no-repeat scroll 5px 10px;border:2px solid #ccc;color:#000;"><img src="/app/img/ajax-loader.gif" />&nbsp;' + msg + '</div>');
        _html.append(_inner);
        _html.css({
            top: offset.top + "px",
            left: offset.left + "px",
