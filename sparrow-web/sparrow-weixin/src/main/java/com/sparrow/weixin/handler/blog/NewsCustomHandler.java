 ...
 */
public class NewsCustomHandler extends CustomHandler {
    @Override
    public Object process(Message message) {
        if (message instanceof TextMessage)
            return this.getNewsResult(message);
        return this.getDefaultReplay();
    }

    WeXinResult getNewsResult(Message message) {
        /** 此处为图灵api接口，参数key需要自己去注册申请，先以11111111代替 */
        String s = String.valueOf(System.currentTimeMillis());
        String apiUrl = "http://api.1-blog.com/biz/bizserver/news/list.do?max_behot_time=" + s + "&size=10";
        //将参数转为url编码
        /** 发送httpget请求 */
        ...
        /** 请求失败处理 */
        if (null == result) {
            return new WeXinResult(MessageType.Text, "猛搓新闻……");
        }

        BlogResult<BlogNews> rt = null;
        try {
            rt = JsonMapper.mapper.readValue(result, new TypeReference<BlogResult<BlogNews>>() {
            });
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (rt != null && StringUtils.equals("000000", rt.getStatus())) {
            List<BlogNews> list = rt.getDetail();
            ...
            int i = 0;
            for (BlogNews item : list) {
                xmlBuilder.append("item");
                xmlBuilder.appendTextNode("Title", item.getTitle());
               if (i == 0)
                    xmlBuilder.appendTextNode("PicUrl", "http://static.dili7.com/static/newStatic/pnr/upload/logo.png");
                xmlBuilder.appendTextNode("Url", item.getArticle_url());
                xmlBuilder.endTag();
                i++;
            }
