...
public class SQLParser {
    ...
    public String rebuildSql(TableConfig cfg) {
        return "SELECT " + (this.columnStr == null ? "*" : this.columnStr)
                + " FROM " + (cfg == null ? this.table : cfg.getName()) + " "
                + replaceColumn(this.fromAft, cfg);
    }

    public String replaceColumn(String str, TableConfig cfg) {
        String nql = str;
        int idx = nql.indexOf('.');
        if (idx != -1) {
            ...
            int ndx;
            while (idx != -1) {
                sb.append(nql.substring(0, idx)).append('.');
                nql = nql.substring(idx + 1);
                ndx = nql.indexOf(' ');
                if (ndx == -1)
                    ndx = nql.indexOf('=');
                if (ndx == -1)
                    ndx = nql.indexOf('<');
                if (ndx != -1) {
                    sb.append(cfg.getColumn(nql.substring(0, ndx)));
                    nql = nql.substring(ndx);
                } else {
                    sb.append(nql);
                    break;
                }
    ...
    public String rebuildCountSql(TableConfig cfg) {
        return "SELECT COUNT(*) FROM "
                + (cfg == null ? this.table : cfg.getName()) + " "
                + replaceColumn(this.fromAft, cfg);
    }
    ...
    public static String getMysqlPagedSql(String sql, int pageIndex,
                                          int pageSize) {
        StringBuffer sb = new StringBuffer();
        if (pageIndex < 1)
            pageIndex = 1;
