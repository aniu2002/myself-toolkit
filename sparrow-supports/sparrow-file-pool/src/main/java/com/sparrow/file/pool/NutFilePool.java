...
import com.sparrow.file.tool.Disks;
...
public class NutFilePool implements FilePool {
    ...
    private static final Logger log = LoggerFactory.getLogger(NutFilePool.class);
    ...
    public NutFilePool(String homePath, long size) {
        if (log.isInfoEnabled()) {
            log.info("Init file-pool by:{} [{}]", homePath, size);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("file-pool.home: '{}'", home.getAbsolutePath());
        }
        ...
        if (log.isInfoEnabled())
            log.info("file-pool.cursor: {}", cursor);
    }
