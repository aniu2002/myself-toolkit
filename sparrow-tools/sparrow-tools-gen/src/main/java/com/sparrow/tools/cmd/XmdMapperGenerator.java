...
import com.sparrow.tools.cmd.meta.PojoTableColumn;
...
public class XmdMapperGenerator {
    ...
    public XmdMapperGenerator(DbSetting dbs) {
        this(dbs, new DefaultLog());
    }

    public XmdMapperGenerator(DbSetting dbs, Log log) {
        this.dbs = dbs;
    ...
    public static File getPackDir(String base, String pack ) {
        File file = new File(base, getFilePath(pack));
        return file;
    }


    public static File getNormalFile(String base, String pack, String name) {
        File file = new File(base, getFilePath(pack));
        if (!file.exists())
            file.mkdirs();
    ...
    static void setColumnType(PojoTableColumn column, int sqlType, int columnSize,
                              int decimalDigits) {
    ...
    public static String toBeanNameX(String field) {
        if ("ID".equalsIgnoreCase(field))
            return "ID";
        if (null == field) {
            return "";
    ...
    private List<PojoTableColumn> getTableItems$Oracle(DatabaseMetaData metadata,
                                                       PojoTable table, String primaryKey, IDAliasGenerator generator) {
        ResultSet rs = null;
        ...
        try {
            ...
            List<PojoTableColumn> columns = new ArrayList<PojoTableColumn>();
            PojoTableColumn item;
            short sqltype;
            int columnLenth, decimalLength;
            // ResultSetMetaData rsmd = rs.getMetaData();
            // int count = rsmd.getColumnCount();
            while (rs.next()) {
                item = new PojoTableColumn();
                item.setName(rs.getString(4));// rs.getString("COLUMN_NAME")
                item.setFieldName(NameRule.toBeanName(item.getName()));
                item.setFieldNameX(toBeanName(item.getName()));
    ...
    public PojoTable getTableDescriptor(String tableName, IDAliasGenerator generator) {
        Connection conn = null;
        // ResultSet rs = null;
        DatabaseMetaData metadata;
        try {
            conn = getConnection();
            metadata = conn.getMetaData();
            PojoTable table = new PojoTable(tableName);
    ...
    private List<PojoTableColumn> getTableItems(DatabaseMetaData metadata,
                                                PojoTable table, String primaryKeys, IDAliasGenerator generator) {
    ...
    private List<SimpleTable> getTables4Oracle() {
        Connection conn = null;
        Statement stmt = null;
        ResultSet rs = null;
        List<SimpleTable> tableNames = new ArrayList<SimpleTable>();
        String table;
        try {
            conn = getConnection();
            ...
            while (rs.next()) {
                ...
                if (this.filters != null && this.filters.filter(table)) {
                    SimpleTable stable = new SimpleTable(table);
    ...
    private List<SimpleTable> getTable4MySql() {
        List<SimpleTable> tableNames = new ArrayList<SimpleTable>();
        Connection conn = null;
        ResultSet rs = null;
        String table;
    ...
    private List<PojoTableColumn> getTableItem$MySql(DatabaseMetaData metadata,
                                                     PojoTable table, String primaryKey, IDAliasGenerator generator) {
        ...
        try {
            rs = metadata.getColumns(this.getCatalog(), this.getSchema(),
                    table.getName(), null);
            List<PojoTableColumn> columns = new LinkedList<PojoTableColumn>();
            PojoTableColumn item;
            short sqltype;
            int columnLenth, decimalLength;
