...
public class CodeGeneratorExl {
    ...
    public static void doGenerate(Properties properties, Properties moduleSet,
        ...
        CodeGeneratorExl poGenerateUtil = new CodeGeneratorExl();
    ...
    public void execute() throws Exception {
        ...
        dm.setFilters(new TableFilters() {
            public boolean filter(String table) {
                return CodeGeneratorExl.this.check(table);
            }
        });
        Connection session = ConnectionHolder.getConnection();
        if (session != null) {
            dm.setConnection(session);
        } else
            dm.checkDatabase();
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            // + table.getPakage() + "." + table.getObjName());
            // DTO
//			 File moduleFile = MapperGenerator.getJavaPackFile(basePath,
//			 subPak,
//			  table.getObjName()+"Dto");
//			  FreeMarkerUtils.getInstance().writeFile("au_Pojo", c,
//			  moduleFile);
        ...
        File ngCmdFile = XmdMapperGenerator.getNormalFile(basePath, "eggs",
                "beanConfig.xml");
