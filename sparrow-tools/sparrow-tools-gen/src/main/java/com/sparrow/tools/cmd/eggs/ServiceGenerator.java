...
import com.sparrow.tools.pogen.*;
...
public class ServiceGenerator {
    ...
    public static void main(String args[]) {
        ...
        ServiceGenerator codeGenerator = new ServiceGenerator();
    ...
    public static void doGenerate(Properties properties, Properties moduleSet,
        ...
        ServiceGenerator poGenerateUtil = new ServiceGenerator();
    ...
    List<MetaField> convert(List<TableColumn> items) {
        List<MetaField> fields = new ArrayList<MetaField>();
        for (TableColumn c : items) {
            MetaField mf = new MetaField();
            mf.setName(c.getName());
            mf.setField(c.getFieldName());
            mf.setJavaType(c.getJavaType());
            mf.setType(c.getType());
            mf.setSimpleType(c.getSampleType());
            mf.setColdef(true);
            mf.setDesc(c.getDesc());
            mf.setNotNull(c.isNotNull());
            mf.setPrimary(c.isPrimary());
            mf.setSearch(false);
            mf.setSize(c.getSize());
            mf.setVtype("required");
            mf.setUi("text");
            fields.add(mf);
        }
        return fields;
    ...
    protected String getAppUrl() {
        if (StringUtils.isEmpty(SystemConfig.APP_URL))
            return "http://127.0.0.1:9081/app";
        return SystemConfig.APP_URL;
    }

    protected String getImgUrl() {
        if (StringUtils.isEmpty(SystemConfig.IMG_URL))
            return "http://127.0.0.1:9081/img";
        return SystemConfig.IMG_URL;
    }
    ...
    public void execute() throws Exception {
        ...
        dm.setFilters(new TableFilters() {
            public boolean filter(String table) {
                return ServiceGenerator.this.check(table);
            }
        });
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            if (!metas.containsKey(tb.toLowerCase())) {
                mdx = new MetaDefine();
                mdx.setTable(table.getName());
                mdx.setSetting(this.convert(table.getItems()));
            } else
                mdx = metas.get(tb.toLowerCase());
        ...
        gv.setServices(srvs);
        gv.setModule(this.getModuleName());
        gv.setBasePackage(basePak);

        File ngCmdFile = this.getServiceConfPath(basePath);

        this.setConfPath(gv, ngCmdFile.getPath());
...
//                "CmdReg");
        ...
        FreeMarkerUtils.getInstance().writeFile("bean_Config", gv, ngCmdFile);
        dm.close();
    ...
    void generateService(String basePackage, String subPackage,
        ...
        // FreeMarkerUtils.getInstance().writeFile("au_Service", c,
        // serviceFile);
    ...
    void generateView(Table table, Map<String, Object> c, String moduleName,
        ...
        c.put("appRoot", this.getAppUrl());
        c.put("imgRoot", this.getImgUrl());
        ...
        this.log.msg("-创建列表页:" + destSource + "/list.html , " + fviewFile.getPath());
        ...
        if (this.mWriter == null)
            this.mWriter = new ModuleWriter(this.getWepPath());
