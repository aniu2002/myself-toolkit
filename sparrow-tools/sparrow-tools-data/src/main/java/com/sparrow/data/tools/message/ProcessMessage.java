...
public class ProcessMessage {
    ...
    public void begin() {
        this.state = 1;
        this.startTime = MessageManager.now();
    }
    ...
    void setCostTime() {
        this.costSeconds = MessageManager.convert(MessageManager.now()
                - this.startTime);
    }
    ...
    public void notify(int step, String msg) {
        int p = this.percent;
        p = p + step;
        if (p < 90)
            this.percent = p;
    ...
    public ProcessStatus fetchData() {
        ...
        if (this.state == 2 || this.state == -1) {
            this.msgs = null;
            MessageManager.removeMessage(this.sid);
        }
        return status;
    }
