 ...
 * @modify
 */
public class Excel2003ModifyDataWriter extends AbstractDataWriter {
	...
	public Excel2003ModifyDataWriter(String template, String excelFile) {
		this(new File(template), new File(excelFile));
	}

	public Excel2003ModifyDataWriter(File templateFile, File excelFile) {
	...
	public void setExpSetting(ExpSetting expSetting) {
		...
		this.currentIdx = expSetting.getStartRow();
	}
	...
	protected void doOpen() throws IOException {
		...
		try {
            ...
            newSettings.setIgnoreBlanks(true);
            newSettings.setSuppressWarnings(true);
            newSettings.setCellValidationDisabled(true);

			WorkbookSettings setting = new WorkbookSettings();
			setting.setSuppressWarnings(true);

			File tmpFile = FileStore.getDatePathRandomFile("tmp");
			FileUtils.copyFile(this.templateFile, tmpFile);
			...
			this.workBook = Workbook
					.createWorkbook(this.excelFile,baseBook, newSettings);
	...
	void importSheet(Workbook baseBook, WritableWorkbook workBook) {
		...
		for (int i = 0; i < numberOfSheets; i++) {
			...
			//workBook.copySheet(s, name, index)
		}
		baseBook.close();
	}
