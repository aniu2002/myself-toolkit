...
/**
 * A transient utility object used to copy sheets. This functionality has been
 * farmed out to a different class in order to reduce the bloat of the
 * WritableSheetImpl
 */
...
class SheetCopier {
	...
	/**
	 * Copies a sheet from a read-only version to the writable version. Performs
	 * shallow copies
	 */
	public void copySheet() {
		...
		for (int i = 0; i < merged.length; i++) {
			mergedCells.add(new SheetRangeImpl((SheetRangeImpl) merged[i],
					toSheet));
		}
		...
		try {
			...
			for (int i = 0; i < rowprops.length; i++) {
				...
				XFRecord format = rowprops[i].hasDefaultFormat() ? formatRecords
						.getXFRecord(rowprops[i].getXFIndex()) : null;
				rr.setRowDetails(rowprops[i].getRowHeight(),
						...
						rowprops[i].getGroupStart(), format);
		...
		for (int i = 0; i < dr.length; i++) {
			if (dr[i] instanceof jxl.biff.drawing.Drawing) {
				WritableImage wi = new WritableImage(dr[i], toSheet
						.getWorkbook().getDrawingGroup());
				drawings.add(wi);
				images.add(wi);
				...
				CellValue cv = (CellValue) toSheet.getWritableCell(
				...
				jxl.biff.drawing.Button b = new jxl.biff.drawing.Button(dr[i],
						toSheet.getWorkbook().getDrawingGroup(),
						workbookSettings);
				drawings.add(b);
			} else if (dr[i] instanceof jxl.biff.drawing.ComboBox) {
				jxl.biff.drawing.ComboBox cb = new jxl.biff.drawing.ComboBox(
						dr[i], toSheet.getWorkbook().getDrawingGroup(),
						workbookSettings);
				drawings.add(cb);
			} else if (dr[i] instanceof jxl.biff.drawing.CheckBox) {
				jxl.biff.drawing.CheckBox cb = new jxl.biff.drawing.CheckBox(
						dr[i], toSheet.getWorkbook().getDrawingGroup(),
						workbookSettings);
				drawings.add(cb);
			}
		...
		if (rdv != null) {
			dataValidation = new DataValidation(rdv, toSheet.getWorkbook(),
					toSheet.getWorkbook(), workbookSettings);
	...
	public void copyWritableSheet() {
		...
		/*
		 * // Copy the column formats Iterator cfit =
		 * fromWritableSheet.columnFormats.iterator(); while (cfit.hasNext()) {
		 * ColumnInfoRecord cv = new ColumnInfoRecord ((ColumnInfoRecord)
		 * cfit.next()); columnFormats.add(cv); }
		 * 
		 * // Copy the merged cells Range[] merged =
		 * fromWritableSheet.getMergedCells();
		 * 
		 * for (int i = 0; i < merged.length; i++) { mergedCells.add(new
		 * SheetRangeImpl((SheetRangeImpl)merged[i], this)); }
		 * 
		 * // Copy the row properties try { RowRecord[] copyRows =
		 * fromWritableSheet.rows; RowRecord row = null; for (int i = 0; i <
		 * copyRows.length ; i++) { row = copyRows[i];
		 * 
		 * if (row != null && (!row.isDefaultHeight() || row.isCollapsed())) {
		 * RowRecord rr = getRowRecord(i); rr.setRowDetails(row.getRowHeight(),
		 * row.matchesDefaultFontHeight(), row.isCollapsed(), row.getStyle()); }
		 * } } catch (RowsExceededException e) { // Handle the rows exceeded
		 * exception - this cannot occur since // the sheet we are copying from
		 * will have a valid number of rows Assert.verify(false); }
		 * 
		 * // Copy the horizontal page breaks rowBreaks = new
		 * ArrayList(fromWritableSheet.rowBreaks);
		 * 
		 * // Copy the vertical page breaks columnBreaks = new
		 * ArrayList(fromWritableSheet.columnBreaks);
		 * 
		 * // Copy the data validations DataValidation rdv =
		 * fromWritableSheet.dataValidation; if (rdv != null) { dataValidation =
		 * new DataValidation(rdv, workbook, workbook, workbookSettings); }
		 * 
		 * // Copy the charts
		 * sheetWriter.setCharts(fromWritableSheet.getCharts());
		 * 
		 * // Copy the drawings DrawingGroupObject[] dr = si.getDrawings(); for
		 * (int i = 0 ; i < dr.length ; i++) { if (dr[i] instanceof
		 * jxl.biff.drawing.Drawing) { WritableImage wi = new
		 * WritableImage(dr[i], workbook.getDrawingGroup()); drawings.add(wi);
		 * images.add(wi); }
		 * 
		 ...
		 * 
		 * // Copy the workspace options
		 * sheetWriter.setWorkspaceOptions(fromWritableSheet
		 * .getWorkspaceOptions());
		 * 
		 * // Copy the environment specific print record if
		 * (fromWritableSheet.plsRecord != null) { plsRecord = new
		 * PLSRecord(fromWritableSheet.plsRecord); }
		 * 
		 * // Copy the button property set if
		 * (fromWritableSheet.buttonPropertySet != null) { buttonPropertySet =
		 * new ButtonPropertySetRecord (fromWritableSheet.buttonPropertySet); }
		 */
	}
	...
	public void importSheet() {
		...
		for (int i = 0; i < readCirs.length; i++) {
			...
			for (int j = rcir.getStartColumn(); j <= rcir.getEndColumn(); j++) {
				...
				XFRecord cf = (WritableCellFormat) xfRecords.get(new Integer(
						xfIndex));

				if (cf == null) {
					CellFormat readFormat = fromSheet.getColumnView(j)
							.getFormat();
		...
		try {
			...
			for (int i = 0; i < rowprops.length; i++) {
				...
				if (rowrec.hasDefaultFormat()) {
					format = (WritableCellFormat) xfRecords.get(new Integer(
							rowrec.getXFIndex()));

					if (format == null) {
						int rownum = rowrec.getRowNumber();
						CellFormat readFormat = fromSheet.getRowView(rownum)
								.getFormat();
						WritableCellFormat wcf = copyCellFormat(readFormat);
					}
				}
				...
				rr.setRowDetails(rowrec.getRowHeight(),
						...
						rowrec.isCollapsed(), rowrec.getOutlineLevel(),
						rowrec.getGroupStart(), format);
				numRows = Math.max(numRows, rowprops[i].getRowNumber() + 1);
			}
		} catch (RowsExceededException e) {
			// Handle the rows exceeded exception - this cannot occur since
		...
		if (fromCharts != null && fromCharts.length > 0) {
			...
			/*
			 * sheetWriter.setCharts(fromSheet.getCharts()); IndexMapping
			 * xfMapping = new IndexMapping(200); for (Iterator i =
			 * xfRecords.keySet().iterator(); i.hasNext();) { Integer key =
			 * (Integer) i.next(); XFRecord xfmapping = (XFRecord)
			 * xfRecords.get(key); xfMapping.setMapping(key.intValue(),
			 * xfmapping.getXFIndex()); }
			 * 
			 * IndexMapping fontMapping = new IndexMapping(200); for (Iterator i
			 * = fonts.keySet().iterator(); i.hasNext();) { Integer key =
			 * (Integer) i.next(); Integer fontmap = (Integer) fonts.get(key);
			 * fontMapping.setMapping(key.intValue(), fontmap.intValue()); }
			 * 
			 * IndexMapping formatMapping = new IndexMapping(200); for (Iterator
			 * i = formats.keySet().iterator(); i.hasNext();) { Integer key =
			 * (Integer) i.next(); Integer formatmap = (Integer)
			 * formats.get(key); formatMapping.setMapping(key.intValue(),
			 * formatmap.intValue()); }
			 * 
			 * // Now reuse the rationalization feature on each chart to //
			 * handle the new fonts for (int i = 0; i < fromCharts.length ; i++)
			 * { fromCharts[i].rationalize(xfMapping, fontMapping,
			 * formatMapping); }
			 */
		}
		...
		for (int i = 0; i < dr.length; i++) {
			if (dr[i] instanceof jxl.biff.drawing.Drawing) {
				WritableImage wi = new WritableImage(dr[i].getX(),
						dr[i].getY(), dr[i].getWidth(), dr[i].getHeight(),
						dr[i].getImageData());
	...
	private WritableCell deepCopyCell(Cell cell) {
		...
		if (c instanceof ReadFormulaRecord) {
			...
			boolean crossSheetReference = !rfr.handleImportedCellReferences(
					fromSheet.getWorkbook(), fromSheet.getWorkbook(),
					workbookSettings);
		...
		WritableCellFormat wcf = (WritableCellFormat) xfRecords
	...
	/**
	 * Perform a shallow copy of the cells from the specified sheet into this
	 * one
	 */
			...
			for (int j = 0; j < row.length; j++) {
				...
				try {
					if (c != null) {
						...
						// Cell.setCellFeatures short circuits when the cell is
						// copied,
						// so make sure the copy logic handles the validated
						// cells
	...
	private WritableCellFormat copyCellFormat(CellFormat cf) {
		try {
			...
			formats.put(new Integer(formatIndex),
					new Integer(f.getFormatRecord()));
	...
	/**
	 * Gets the number of rows - allows for the case where formatting has been
	 * applied to rows, even though the row has no data
	 * 
