...
public final class AopMethodAdapter extends NullMethodAdapter {
	...
	public AopMethodAdapter(MethodVisitor mv, int access, String methodName,
			String desc, int methodIndex, String myName,
	...
	private void enhandMethod_Void() {
		...
		loadArgsAsArray();
		mv.visitMethodInsn(INVOKESPECIAL, myName, "$_before",
		...
		loadArgsAsArray();
		mv.visitMethodInsn(INVOKESPECIAL, myName, "$_after",
		...
		loadArgsAsArray();
		mv.visitMethodInsn(INVOKESPECIAL, myName, "$_Exception",
		...
		mv.visitVarInsn(ALOAD, lastIndex);
		loadArgsAsArray();
		mv.visitMethodInsn(INVOKESPECIAL, myName, "$_Error",
	...
	private void enhandMethod_Object() {
		...
		mv.visitVarInsn(ASTORE, lastIndex);
		mv.visitLabel(l4);
		mv.visitFrame(Opcodes.F_APPEND, 1, new Object[] { "java/lang/Object" },
				0, null);
		mv.visitVarInsn(ALOAD, 0);
		mv.visitIntInsn(SIPUSH, methodIndex);
		mv.visitVarInsn(ALOAD, lastIndex);
		...
		returnIt();
		mv.visitLabel(l2);
		mv.visitFrame(Opcodes.F_FULL, 1, new Object[] { myName }, 1,
				new Object[] { "java/lang/Exception" });
		mv.visitVarInsn(ASTORE, lastIndex);
		mv.visitVarInsn(ALOAD, 0);
		mv.visitIntInsn(SIPUSH, methodIndex);
	...
	private void unpackagePrivateData(Type type) {
		if (type.equals(Type.BOOLEAN_TYPE)) {
			mv.visitMethodInsn(INVOKESTATIC, "com/sparrow/core/aop/Helper",
