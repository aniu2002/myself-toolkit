...
public class FtpIoSession implements IoSession {
	...
	public static final String ATTRIBUTE_UDP_DATA_CONNECTION = ATTRIBUTE_PREFIX
			+ "data-udp-connection";// 增加udp 数据链路
	...
	public synchronized ServerDataConnectionFactory getUdpDataConnection() {
		if (containsAttribute(ATTRIBUTE_UDP_DATA_CONNECTION)) {
			return (ServerDataConnectionFactory) getAttribute(ATTRIBUTE_UDP_DATA_CONNECTION);
		} else {
			IOUdpDataConnectionFactory dataCon = new IOUdpDataConnectionFactory(
					context, this);
			dataCon
					.setServerControlAddress(((InetSocketAddress) getLocalAddress())
							.getAddress());
			setAttribute(ATTRIBUTE_UDP_DATA_CONNECTION, dataCon);
			return dataCon;
		}
	}
