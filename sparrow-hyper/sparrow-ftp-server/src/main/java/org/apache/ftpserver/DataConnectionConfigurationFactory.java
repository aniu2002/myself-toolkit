...
public class DataConnectionConfigurationFactory {
	...
	private Logger log = LoggerFactory
			.getLogger(DataConnectionConfigurationFactory.class);

	// maximum idle time in seconds
	private int idleTime = Integer.valueOf(System.getProperty(
			FtpConstants.TimeOut_Key, "10")) * 1000;// 配置时以分钟为单位
	...
	public DataConnectionConfiguration createDataConnectionConfiguration() {
		checkValidAddresses();
		return new DefaultDataConnectionConfiguration(idleTime, ssl,
				activeEnabled, activeIpCheck, activeLocalAddress,
				activeLocalPort, passiveAddress, passivePorts,
				passiveExternalAddress, implicitSsl);
	}

	/*
	 * (Non-Javadoc) Checks if the configured addresses to be used in further
	 ...
	 * 
	 * @param activeIpCheck
	 *            true if the PORT IP should be verified
	 */
	 ...
	 *            The address for active connections
	 */
	 ...
	 *            The address used for passive connections
	 */
	 ...
	 *            The passive address to be returned to clients
	 */
	...
	public synchronized int requestPassivePort() {
		...
		while ((dataPort == -1) && (--loopTimes >= 0)
			...
			if (dataPort == -1) {
				try {
					log
							.info("Out of passive ports, waiting for one to be released. Might be stuck");
					wait();
