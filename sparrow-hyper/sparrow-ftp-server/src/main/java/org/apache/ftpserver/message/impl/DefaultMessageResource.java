...
public class DefaultMessageResource implements MessageResource {
	 ...
	 * {@link MessageResourceFactory} instead.
	 */
	public DefaultMessageResource(List<String> languages,
		...
		String defalutLanguage = locale.getLanguage();
		defalutLanguage = null;
		PropertiesPair pair = createPropertiesPair(defalutLanguage,
				customMessageDirectory);
		messages.put(defalutLanguage, pair);
	...
	private String getWellComeString(String defalut) {
		return System.getProperty(FtpConstants.Welcome_Key, defalut);
	}

	private String getQuitString(String defalut) {
		return System.getProperty(FtpConstants.Quit_Key, defalut);
	}
	...
	public String getMessage(int code, String subId, String language) {
		...
		if (code == FtpReply.REPLY_220_SERVICE_READY)// 对欢迎信息增加可配置
			return getWellComeString(value);
		else if (code == FtpReply.REPLY_221_CLOSING_CONTROL_CONNECTION)// 对欢迎信息增加可配置
			return getQuitString(value);
