...
public class IOUdpDataConnectionFactory implements ServerDataConnectionFactory {
	...
	private IOUdpDataConnection dataConnect;

	public IOUdpDataConnectionFactory(final FtpServerContext serverContext,
	...
	public IOUdpDataConnection openConnection() throws Exception {
		dataConnect = new IOUdpDataConnection();
		dataConnect.setDatagramSocket(createDataSocket());
		return dataConnect;
	}

	public IOUdpDataConnection getDataConnect() {
		return dataConnect;
	}
	 ...
	 */
	private synchronized DatagramSocket createDataSocket() throws Exception {
		...
		try {
			if (!passive) {
				...
				dataSoc = new DatagramSocket(this.port);
				...
				dataSoc
						.connect(new InetSocketAddress(address, this.clientPort));
			} else {
				LOG.debug("Opening passive data connection");
				// System.out.println("init local listener ====== : " +
				// this.port);
				dataSoc = new DatagramSocket(this.port);
				InetAddress localAddr = resolveAddress(dataConfig
						.getActiveLocalAddress());
	...
	public FtpIoSession getFtpIoSession() {
		return this.session;
	}
