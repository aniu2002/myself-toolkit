...
public class ClientController extends OProcess {
	...
	public ClientController() {
		...
		// ClientController.setApplication" );
	...
	public void connect(SocketChannel channelX) {
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.connect " + channelX + " " + closed);
		if (!closed) {
	...
	public void connectFailed() {
		...
		// System.out.println( System.currentTimeMillis() + " " + id + "
		// ClientController.connectFailed " );
	...
	public synchronized void close() {
		...
		if (!socketController.closed)
			socketController.close("detach");
	...
	public void dispatchLeave() {
		// System.out.println( System.currentTimeMillis() + " " + id + "
		// ClientController.dispatchLeave " );
		// Syncing is needed, multiple clients from multiple threads can
	...
	public void dispatchClosure() {
		// System.out.println( System.currentTimeMillis() + " " + id + "
		// ClientController.dispatchClosure " );
	...
	/**
	 * Synchronized packet pushing, multiple client threads can trigger this
	 * function
	 * 
	 * @param packetX
	 *            RtmpPacket
	...
	public void addOutgoingPacket(RtmpPacket packetX) {
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.addOutgoingPacket " + packetX );
	...
	/**
	 * Execution step syncing is needed from previous function
	 */

	public void step() {
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.step " + outgoingList.size() );
		synchronized (outgoingList) {
			// packets exchange
			...
			try {
				// decode packets
				// System.out.println("have a code:"+incomingList );
	...
	public void update() {
		...
		if (accepted) {
			// sending out ping
			if (currentTime - lastPing > Library.PINGTIME)
				sendPing(6, (int) lastPing & 0xffffffff, -1, -1);
			// updating bandwidth information
			if (currentTime - lastBand > Library.BANDTIME)
				updateBand(currentTime - lastBand);
		}
	}
	 ...
	 * 
	 * @param delayX
	 *            time delay since last band update
	 */

	public void updateBand(long delayX) {

		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.updateBand " + socketController.bytesIn );
		...
		if (bytesIn - lastRead > readStepClient)
			sendRead();
	 ...
	 * 
	 * @param packetX
	 *            RtmpPacket packet to sort
	 */

	public void receivePacket(RtmpPacket packetX) {

		// System.out.println( System.currentTimeMillis() + " " + id + "
		// ClientController.receivePacket " + packetX );
		// System.out.println(" Rtmp Packet:" + packetX.bodyType);
		switch (packetX.bodyType) {
		// 0x03 - Read amount
		// 0x04 - Ping received
		// 0x05 - Server or down loading bandwidth
		// 0x06 - Client or uploading bandwidth
		// 0x12 - Stream meta data
		// 0x14 - Invoke

		case 0x03:
			// System.out.println("- 0x03 Read amount ");
			receiveRead(packetX);
			break;
		case 0x04:
			// System.out.println("- 0x04 Ping received ");
			receivePing(packetX);
			break;
		case 0x05:
			// System.out.println("- 0x05 Server or down loading bandwidth ");
			receiveReadStepClient(packetX);
			break;
		case 0x06:
			// System.out.println("- 0x06 Client or up loading bandwidth ");
			receiveReadStepServer(packetX);
			break;
		case 0x12:
			// System.out.println("- 0x12 Stream meta data ");
			receiveInvoke(packetX);
			break;
		case 0x14:
			// System.out.println("- 0x14 Invoke ");
			receiveInvoke(packetX);
			break;
		default:
			receiveUnknown(packetX);
			break;
	...
	public void sendRead() {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.sendRead " + bytesIn );
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.sendRead " + Encoder.getHexa( packet.body ) );
	...
	/**
	 * Receives read notification from client, have to compare with our write
	 * amount, if there is a big difference, we have to decrease data output
	 * 
	 * @param packetX
	 *            RtmpPacket incoming packet
	 */

	public void receiveRead(RtmpPacket packetX) {

		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.receiveRead " + packetX );

		long clientRead = Encoder.bytesToInt(packetX.body);

		// System.out.println( System.currentTimeMillis() + " clientRead: " +
		// clientRead + " difference: " + difference );

		// if difference is big, drop frames

		if (clientRead < (bytesOut - 10000))
			streamController.addDropping();

		// if difference is small, remove dropping

		if (clientRead >= (bytesOut - 10000))
			streamController.removeDropping();
	...
	public void sendPing(int typeX, int p1X, int p2X, int p3X) {
		if (this.logger.isEnable())
			logger.debug(System.currentTimeMillis() + " " + id
					+ " ClientController.sendPing " + typeX + " " + p1X + " "
					+ p2X + " " + p3X);
		// checking ping timeout
		if (ping > Library.PINGTIMEOUT)
			detach();
		else {
	...
	/**
	 * Receives ping from client receivePing : rtmp flow control ping uzenetek.
	 * elso ket byte a tipus. 00 - stream reset , 01 - stream buffer clear, 03 -
	 * stream buffer meret beallitas , 06 - ping , 07 -pong, 08 - first ping
	 * talan
	 * 
	 * @param packetX
	 */
	...
	public void receivePing(RtmpPacket packetX) {
		if (this.logger.isEnable())
			logger.debug(System.currentTimeMillis() + " " + id
					+ " RTMPFlowController.receivePing " + packetX);
		byte[] body = packetX.body;
		// extracting parts from body
		int type = Encoder.bytesToInt(new byte[] { body[0], body[1] });
		int part1 = Encoder.bytesToInt(new byte[] { body[2], body[3], body[4],
				body[5] });
		int part2 = body.length > 6 ? Encoder.bytesToInt(new byte[] { body[6],
				body[7], body[8], body[9] }) : 0;
		int part3 = body.length > 10 ? Encoder.bytesToInt(new byte[] {
				body[10], body[11], body[12], body[13] }) : 0;
		...
		// normal ping request, sending ping

		case 6:
			sendPing(7, part1, part2, part3);
			break;

		// normal ping, checking roundtrip delay
		...
		// default : System.out.println( "Ping: " + Encoder.getHexa(
		// packetX.body ) ); break;
	...
	public void sendReadStepServer(int stepX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.sendReadStepServer " );
	...
	public void receiveReadStepClient(RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + id + "
		// ClientController.receiveReadStepClient " + packetX );
	...
	public void sendReadStepClient(int multiX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.sendReadStepClient " );

		RtmpPacket packet = new RtmpPacket();
		packet.bodyType = 0x06;
		packet.rtmpChannel = 0x02;
		packet.body = Encoder.concatenate(Encoder.intToBytes(multiX, 4),
				new byte[] { 0x02 });
	...
	public void receiveReadStepServer(RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + id + "
		// RTMPFlowController.receiveReadStepServer " + packetX );
		// System.out.println( Encoder.bytesToInt( packetX.body ) );
	...
	public void receiveUnknown(RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + id + "Unknown
		// RtmpPacket " + packetX.toString( ) );
	...
	private void receiveInvoke(RtmpPacket packetX) {

		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.receiveInvoke " );
		...
		try {
			...
			if (this.logger.isEnable())
				logger.debug("command: " + invokeId);

			if (invokeId.equals("receiveAudio"))
				streamController.onAudioReceiveState(arguments, packetX);
			else if (invokeId.equals("receiveVideo"))
				streamController.onVideoReceiveState(arguments, packetX);
			else if (invokeId.equals("createStream"))
				streamController.onStreamCreateRequest(arguments, packetX);
			else if (invokeId.equals("deleteStream"))
				streamController.onStreamDeleteRequest(arguments, packetX);
			else if (invokeId.equals("closeStream"))
				streamController.onStreamCloseRequest(arguments, packetX);
			else if (invokeId.equals("play"))
				streamController.onStreamPlayRequest(arguments, packetX);
			else if (invokeId.equals("pauseRaw"))
				streamController.onStreamPauseRequest(arguments, packetX);
			else if (invokeId.equals("pause"))
				streamController.onStreamPauseRequest(arguments, packetX);
			else if (invokeId.equals("seek"))
				streamController.onStreamSeekRequest(arguments, packetX);
			else if (invokeId.equals("speed"))
				streamController.onStreamSpeedRequest(arguments, packetX);
			else if (invokeId.equals("publish"))
				streamController.onStreamPublishRequest(arguments, packetX);
			else if (invokeId.equals("measure"))
				onMeasure(arguments, packetX);
			else if (invokeId.equals("_error"))
				onResult(arguments, packetX);
			else if (invokeId.equals("_result"))
				onResult(arguments, packetX);
			else if (invokeId.equals("onStatus"))
				onStatus(arguments, packetX);
			else if (invokeId.equals("connect"))
				onConnect(arguments);
			else
				onInvoke(arguments);
			...
			if (this.logger.isEnable())
				logger.debug(Library.CODEEX);
			exception.printStackTrace();

			// create status info

			WrapperMap status = new WrapperMap(Library.STATUSKEYS,
					Library.AMFERRORARR);
	 ...
	 * 
	 * @param argumentsX
	 *            Conneciton arguments
	 */
	public void onConnect(WrapperList argumentsX) {

		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.onConnect " + argumentsX.size( ) );
		...
		argumentsX.remove(0);
		// client.accept();
		this.accept(new Wrapper());

	}

//	public void onConnect_1(WrapperList argumentsX) {
//
//		// System.out.println( System.currentTimeMillis( ) + " " + id + "
//		// ClientController.onConnect " + argumentsX.size( ) );
//
...
//
//			if (this.logger.isEnable())
//				logger.debug("parameter:" + argumentsX.getString(0));
//			// enter client, sync is needed because multiple client threads can
//			// enter an application
//
//			synchronized (application) {
//				application.onEnter(client, argumentsX);
//			}
//
//		} else {
//			argumentsX.remove(0);
//			argumentsX.remove(0);
//			argumentsX.remove(0);
//			String ctp = argumentsX.getString(0);
//			if (this.logger.isEnable())
//				logger.debug("cssData:" + ctp);
//			if ("StreamControl".equals(ctp)) {
//				client.accept();
//				return;
//			}
//			if (this.logger.isEnable())
//				logger.debug(argumentsX.getString(0));
//			// sendReadStepServer( readStepServer );
//			// sendReadStepClient( readStepClient );
//
//			// first ping event
...
//			// sendPing( 8 , 0 , -1 , -1 );
//			// sendPing( 0 , 0 , -1 , -1 );
//			reject(new Wrapper(Library.NOINST + customId));
//		}
//
//	}
	...
	public void onInvoke(WrapperList argumentsX) {
		...
		if (invokeListener != null) {
			...
			// System.out.println( System.currentTimeMillis( ) + " " + id + "
			// ClientController.onInvoke " + invokeId );
			...
			if (invokeChannel != 0)
				outgoingInvokes.put(invokeChannel, invokeId);
			invokeListener.onEvent(new InvokeEvent(invokeId, invokeChannel,
					client, argumentsX));
	...
	public void onResult(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.onResult " + invokeChannel + " " + invokeId );
	...
	public void onStatus(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.onStatus " + argumentsX );
	...
	/**
	 * Bandwidth meause request from client, storing read time and create string
	 * if needed
	 * 
	 * @param argumentsX
	 *            arguments
	...
	public void call(String callIdX) {
		invoke(callIdX, 0, 0, new WrapperList());
	}

	public void call(String callIdX, Wrapper argumentX) {
		invoke(callIdX, 0, 0, new WrapperList(argumentX));
	}

	public void call(String callIdX, Wrapper argumentX, int flvChannelX) {
		invoke(callIdX, 0, flvChannelX, new WrapperList(argumentX));
	}

	public void call(String callIdX, WrapperList argumentsX) {
		invoke(callIdX, 0, 0, argumentsX);
	}

	public void call(String callIdX, WrapperList argumentsX, int flvChannelX) {
		invoke(callIdX, 0, flvChannelX, argumentsX);
	}

	public void callResult(double channelX, Wrapper argumentX) {
		invoke("_result", channelX, 0, new WrapperList(argumentX));
	}

	public void callResult(double channelX, WrapperList argumentsX) {
		invoke("_result", channelX, 0, argumentsX);
	}

	public void callResponse(String callIdX, WrapperList argumentsX,
			int flvChannelX) {
	...
	public void invoke(String callIdX, double invokeChannelX, int flvChannelX,
			WrapperList argumentsX) {

		// System.out.println( System.currentTimeMillis() + "
		// ClientController.invoke " + callIdX + " " + invokeChannelX );
	 ...
	 * 
	 * @param wrapperX
	 *            - an information object passed to the client
	 */

	public void accept(Wrapper wrapperX) {

		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.accept" + closed + " " + mode );

		if (!closed && mode.equals("passive")) {

			// create acception status

			WrapperMap info = new WrapperMap(Library.STATUSKEYS,
					Library.SUCCESSARR);
	...
	/**
	 * Rejects the client, called from a custom applications. Sends a standard
	 * rejection object as onStatus
	 * 
	 * @param wrapperX
	 *            - an information object passed to the client
	 */
	...
	public void reject(Wrapper wrapperX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + id + "
		// ClientController.reject closed: " + closed + " mode: " + mode );
	...
	public void addStreamEventListener(EventListener streamListenerX) {
		streamListener = streamListenerX;
	}

	public void addInvokeEventListener(EventListener invokeListenerX) {
		invokeListener = invokeListenerX;
	}

	public void addStatusEventListener(EventListener statusListenerX) {
		statusListener = statusListenerX;
	}

	public HashMap<Integer, Stream> getPlayers() {
		if (!closed)
			return streamController.getPlayers();
		else
			return null;
	}
	...
	public HashMap<Integer, Stream> getRouters() {
		if (!closed)
			return streamController.getRouters();
		else
			return null;
	}
