...
public class Stream {
	...
	public Stream(String nameX, Client clientX) {
		...
		// System.out.println( System.currentTimeMillis( ) +
		// " Stream.construct " + nameX + " " + clientX );
	...
	/**
	 * Plays a local stream called nameX under the name given in constructor (
	 * cloning ) or Plays the nameX flv from the streams directory under the
	 * name given in constructor Plays a remote stream called nameX under the
	 * name given in cosntructor
	 */

	public void play(String nameX) {

		// System.out.println( System.currentTimeMillis( ) + " Stream.play " +
		// nameX + " name: " + name + " client: " + client );

		if (client != null) {

			// if we are remote stream, set name and type and try to start
			// playing
	...
	/**
	 * Publishes a local stream called nameX to a remote server under the name
	...
	public void publish(String nameX, String modeX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " Stream.publish "
		// + nameX + " mode: " + modeX + " name: " + name + " client: " + client
		// );
	...
	public void delete() {
		...
		// System.out.println( System.currentTimeMillis( ) + " Stream.delete " +
		// name );

		if (client != null)
			client.streamController.deleteStream(remoteId, this);
		if (reader != null)
			reader.close();
		if (player != null)
			player.close();
	...
	public void enable() {
		...
		// System.out.println( System.currentTimeMillis( ) + " Stream.enable " +
		// name );
	...
	public void disable() {
		Logger.getLogger().debug(
				System.currentTimeMillis() + " Stream.disable " + name);
		if (router != null)
			router.disable();
		if (player != null)
			player.disable();
	...
	public void pause() {
		...
		// System.out.println( System.currentTimeMillis( ) + " Stream.pause " +
		// name );
		if (reader != null)
			reader.stop();
	...
	public void record(boolean stateX) {
		...
		// " StreamRouter.record " + stateX );
		/*
		 * if ( stateX && writer == null ) writer = new StreamWriter( name ,
		 * false ); if ( !stateX && writer != null ) writer.close( ); if (
		 * !stateX ) writer = null;
		 */
	...
	public void seek(int positionX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " Stream.seek " +
		// name + " " + positionX );
		if (reader != null)
			reader.setPosition(positionX);
		if (client != null)
			client.call("seek", new WrapperList(new Wrapper(positionX)),
					remoteChannel);
