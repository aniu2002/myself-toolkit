 ...
 * @version 20080315
 * 
 * Tasks of Streamcontroller - control stream creation, deletion - control play,
 * publish request - receive and dispatch stream packets
 * 
 */
...
public class StreamController extends OProcess {
	...
	// remotes - stream players pushing stream to a remote client after
	// acception
	...
	public StreamController(ClientController clientX, SocketController socketX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + clientX.id +
		// " StreamController.construct" );
	...
	public int getChannel() {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + clientX.id +
		// " StreamController.getChannel" );

		// for better header scattering, streams should vary avaliable rtmp
		// channels
	...
	public void close() {
		...
		ArrayList<StreamPlayer> playersCopy = new ArrayList<StreamPlayer>(
				players.values());
		ArrayList<StreamRouter> routersCopy = new ArrayList<StreamRouter>(
				routers.values());

		for (StreamPlayer player : playersCopy)
			player.close();
		for (StreamRouter router : routersCopy)
			router.close();
	...
	public void removePlayer(StreamPlayer playerX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.removePlayer " + playerX );
	...
	public void removeRouter(StreamRouter routerX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.removeRouter " + routerX );
	...
	public void onStreamCreateRequest(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.onStreamCreateRequest " + argumentsX.get( 1
		// ).doubleValue );
	...
	public void onStreamDeleteRequest(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.onStreamDeleteRequest " + argumentsX );
	...
	public void onStreamCloseRequest(ArrayList<Wrapper> argumentsX,
			RtmpPacket packetX) {

		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.onStreamDeleteRequest " + argumentsX );

		// creating clone lists, because request only contains the flv channel
		// of closable stream
		...
		for (StreamRouter router : routersCopy)
			if (router.getFlvChannel() == packetX.flvChannel) {
	...
	public void onStreamPlayRequest(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// StreamController.onStreamPlayRequest " + argumentsX.getString( 3 ) );
		int id;
		String name = argumentsX.getString(3);
		Logger.getLogger().debug(
				"StreamController.onStreamPlayRequest  ..... \r\n Play File Name : "
						+ name);
		// close old player
		...
		if (name != null) {
			...
			StreamPlayer player = new StreamPlayer(id, flvChannel,
					videoChannel, audioChannel, name, this);

			players.put(id, player);

			if (client.streamListener == null)
				player.enable();
			else
				client.streamListener.onEvent(new StreamEvent(
						new Stream(player), client.client));
	...
	public void onStreamPublishRequest(WrapperList argumentsX,
			RtmpPacket packetX) {

		// System.out.println( System.currentTimeMillis( ) + "
		// StreamController.onStreamPublishRequest " + argumentsX.getString( 3 )
		// );
		...
		if (name != null) {
			...
			int flvChannel = packetX.flvChannel;
			String mode = argumentsX.size() > 4 ? argumentsX.getString(4)
					: "live";

			StreamRouter router = new StreamRouter(id, flvChannel, name, mode,
					this);
			...
			if (client.streamListener == null)
				...
				client.streamListener.onEvent(new StreamEvent(stream,
						client.client));
	...
	public void createStream(Stream streamX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.createStream " + streamX );
	...
	public void deleteStream(double streamIdX, Stream streamX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.deleteStream " + streamIdX + " " + streamX );

		if (streamX.remoteType.equals("play"))
			stopRemote((int) streamIdX);
		if (streamX.remoteType.equals("publish"))
			unpublishRemote((int) streamIdX);
	...
	public void onCreateStream(double streamIdX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.onCreateStream " + streamIdX );
		...
		// if stream is a remote player, create a related router for incoming
		// stream
		// if stream is a remote publisher, create a player for utgoing stream
		// boradcasting

		if (stream.remoteType.equals("play"))
			playRemote((int) streamIdX, stream.remoteChannel,
					stream.remoteName, stream.name);
		if (stream.remoteType.equals("publish"))
			publishRemote((int) streamIdX, stream.remoteChannel,
					stream.remoteName, stream.name, stream.mode);
	...
	public void playRemote(int idX, int channelX, String remoteNameX,
			String localNameX) {

		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.playRemote " + idX + " " + remoteNameX + " " +
		// localNameX );
		...
		StreamRouter router = new StreamRouter(newId, channelX, localNameX,
				"live", this);
		...
		client
				.call("play", new WrapperList(new Wrapper(remoteNameX)),
						channelX);
	...
	public void stopRemote(int idX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.stopRemote " + idX );
	 ...
	 *            stream id
	 * @param nameX
	 *            local name of stream
	 * @param modeX
	 *            mode of publishing
	 */

	public void publishRemote(int idX, int channelX, String remoteNameX,
			String localNameX, String modeX) {

		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.publishRemote " + idX + " " + remoteNameX + " " +
		// localNameX );
		...
		StreamPlayer player = new StreamPlayer(newId, channelX, videoChannel,
				audioChannel, localNameX, this);
	...
	public void unpublishRemote(int idX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.publishRemote " + idX );
	...
	public void onAudioReceiveState(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.onAudioReceiveState " );

		boolean state = argumentsX.getBoolean(3);
		System.out.println(" - Receive Audio : " + state);
		for (StreamPlayer player : players.values())
			if (player.getFlvChannel() == packetX.flvChannel)
				player.enableAudio(state);
	...
	public void onVideoReceiveState(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.onVideoReceiveState " );
		boolean state = argumentsX.getBoolean(3);
		System.out.println(" - Receive Video : " + state);
		for (StreamPlayer player : players.values())
			if (player.getFlvChannel() == packetX.flvChannel)
				player.enableVideo(state);
	...
	public void onStreamSeekRequest(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id +
		// "StreamController.onStreamSeekRequest" + packetX.toString( ) + " " +
		// argumentsX.getDouble( 3 ) );
		double pos = argumentsX.getDouble(3);
		System.out.println(" Seek Position : " + pos);
		for (StreamPlayer player : players.values())
			if (player.getFlvChannel() == packetX.flvChannel)
				player.reader.setPosition(pos);
	...
	public void onStreamSpeedRequest(WrapperList argumentsX, RtmpPacket packetX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id +
		// "StreamController.onStreamSpeedRequest" + packetX.toString( ) + " " +
		// argumentsX.getString( 3 )+ " " + argumentsX.getDouble( 4 ) );
	...
	public void setBufferLength(double streamIdX, int bufferLengthX) {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.setBufferLength " + streamIdX + " " + bufferLengthX
		// );
	...
	public void step() {
		...
		// System.out.println( System.currentTimeMillis( ) + " " + client.id + "
		// StreamController.step ogList: " + outgoingList.size( ) );
		...
		for (StreamPlayer player : players.values())
			player.givePackets(outgoingList);
	...
	public void addDropping() {
		...
		if (!dropDisposables)
			dropDisposables = true;
		else if (!dropInterframes)
			dropInterframes = true;
		else if (!dropKeyframes)
			dropKeyframes = true;
		else if (!dropAudio)
			dropAudio = true;
	...
	public HashMap<Integer, Stream> getPlayers() {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.getPlayedStreams" );

		HashMap<Integer, Stream> result = new HashMap<Integer, Stream>();
		// for ( Stream player : players.values( ) ) result.put( player.id ,
		// player );
	...
	public HashMap<Integer, Stream> getRouters() {
		...
		// System.out.println( System.currentTimeMillis() + " " + client.id + "
		// StreamController.getPublishedStreams " );

		HashMap<Integer, Stream> result = new HashMap<Integer, Stream>();
		// for ( Stream router : routers.values( ) ) result.put( router.id ,
		// router );
