...
public class SocketConnector extends OProcess {
	...
	public SocketConnector() {
		...
		try {
			...
			Logger.getLogger().debug("Server bind port: " + Library.PORT);
			...
			synchronized (sockets) {
				for (SocketChannel socket : sockets.keySet()) {
					...
					try {
						...
						System.out.println(Library.OPENEX
				...
				for (SocketChannel socket : accepted)
					sockets.get(socket).connect(socket);
				for (SocketChannel socket : rejected)
					sockets.get(socket).connectFailed();
	 ...
	 *            url of server
	 * @param controllerX
	 *            clientcontroller assigned to the request
	 */

	public void connect(String hostX, ClientController controllerX) {

		// System.out.println( System.currentTimeMillis( ) +
		// " SocketConnector.connect " + hostX + " " + controllerX );
		...
		try {
			...
			if (ports.length > 1)
				port = new Integer(ports[ports.length - 1]);
