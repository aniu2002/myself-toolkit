...
public class StreamReader extends OProcess {
	...
	public StreamReader(String fileNameX, OStream subscriberX) {
		if (logger.isEnable())
			logger.debug(System.currentTimeMillis()
					+ " StreamReader.construct " + fileNameX + " "
					+ subscriberX);
		// create
		try {
			streamFile = new RandomAccessFile(new File(Library.STREAMDIR,
					fileNameX), "r");
		...
		try {
			...
			do {
				...
				if (bodyType != -1) {
					...
					bodySize = streamFile.read() << 16 | streamFile.read() << 8
							| streamFile.read();
					flvStamp = streamFile.read() << 16 | streamFile.read() << 8
							| streamFile.read();
	...
	public void setSpeed(double multiplyX) {
		...
		// System.out.println( System.currentTimeMillis( ) +
		// " StreamReader.setSpeed " + multiplyX );
	...
	public void setPosition(double positionX) {
		...
		// System.out.println( System.currentTimeMillis( ) +
		// " StreamReader.setPosition " + positionX );

		if (positionX < 0)
			positionX = 0;
		else if (positionX > duration)
			positionX = duration;
	...
	public void sendStart() {
		...
		// data start for play head
		subscriber.take(RtmpFactory.dataStart());

		// duration
		subscriber.take(RtmpFactory.streamDuration(duration
				/ Math.abs(multiply)));
	...
	public void sendSeek() {
		...
		// seek notify
		subscriber.take(RtmpFactory.seekNotify((int) position, subscriber
				.getId(), subscriber.getClientId(), name));
