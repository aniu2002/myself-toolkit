 ...
 */
public class CrawlerCountListener implements Listener {
    static final Logger logger = LoggerFactory.getLogger(CrawlerCountListener.class);
    ...
    public void crawlerEnd(CrawlerConfig crawlerConfig) {
        if (logger.isInfoEnabled())
            logger.info("退出抓取站点:{}-{}，完成 site={}个，page={}个，detail={}个, store={}个, rows={}个", new Object[]{
    ...
    public void siteEnd(SiteEntry siteEntry) {
        if (logger.isDebugEnabled())
            logger.debug("完成抓取site入口url:{} - {}", siteEntry.getTitle(), siteEntry.getUrl());
    }
    ...
    public void formatEnd(CrawlerData crawlerData, Object data) {
        ...
        if (logger.isDebugEnabled())
            logger.debug("完成数据格式化:{} - {}", crawlerData.getUrl(), data);
    }

    @Override
    public void storeEnter(CrawlerData crawlerData, Object data) {
        if (logger.isDebugEnabled())
            logger.debug("开始数据存储:{} - {}", crawlerData.getUrl(), data);
    }
    ...
    public void storeEnd(CrawlerData crawlerData, Object data, int effects) {
        ...
        if (logger.isDebugEnabled())
            logger.debug("完成数据存储:{} - {}", crawlerData.getUrl(), data);
    }
}
